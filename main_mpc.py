...
parser.add_argument('--time_horizon', type=int, default=120, metavar='S',
                    help='matching steps in the future for demand and arriving vehicle forecast (default: 120 min)')
...
parser.add_argument('--policy_name', type=str, default='mpc',
                    help='Pretrained agent selection for agent evaluation (default: mpc)')
...
parser.add_argument('--three_level', default=False, action='store_true',
                    help='Select if to use a three level optimization framework (default: False)')
...
# Define the AMoD simulator environment and the policy
...
mpc = MPC(env, CPLEXPATH, T=args.time_horizon)
if 'meso' in net_file:
...
for episode in epochs:
    ...
    episode_revenue = 0
    # Intialize flows in case of three-level
    if args.three_level:
        pax_flow = dict()
        reb_flow = dict()
        desired_acc = dict()
    ...
    while (not done):
        ...
        if --args.three_level:
            desired_acc[env.time], pax_flow[env.time], reb_flow[env.time] = mpc.tri_level()
            obs, paxreward, done, info = env.pax_step(CPLEXPATH=CPLEXPATH, PATH='mpc/scenario_lux_test')
            reb_action = solveRebFlow(env, f'scenario_lux/{args.policy_name}', desired_acc[env.time], CPLEXPATH)
        else:
            pax_action, reb_action = mpc.MPC_exact()
            obs, paxreward, done, info = env.pax_step(paxAction=pax_action, CPLEXPATH=CPLEXPATH)
...
print(f"MPC: Reward {log_stat['test_reward']['mean']:.2f}, Revenue {log_stat['test_revenue']['mean']},Served demand {log_stat['test_served_demand']['mean']:.2f}, Rebalancing Cost {log_stat['test_reb_cost']['mean']:.2f}, Operational Cost {log_stat['test_op_cost']['mean']:.2f}, Rebalanced Vehicles {log_stat['test_reb_vehicles']['mean']:.2f}")
print(f"MPC: Reward {log_stat['test_reward']['std']:.2f}, Revenue {log_stat['test_revenue']['std']:.2f},Served demand {log_stat['test_served_demand']['std']:.2f}, Rebalancing Cost {log_stat['test_reb_cost']['std']:.2f}, Operational Cost {log_stat['test_op_cost']['std']:.2f}, Rebalanced Vehicles {log_stat['test_reb_vehicles']['std']:.2f}")
